...
{
	CDialogEx::DoDataExchange(pDX);
	DDX_Text(pDX, IDC_EDIT_MSGINPUT, m_strMsgInput);
	DDX_Control(pDX, IDC_LIST_MSGOUT, m_List);
}
...
BOOL CSocketClientDlg::OnInitDialog()
{
	...
	CString LogInMsg;
	int CODE = 0;
	LogInMsg.Format(_T("%d %s"),CODE, UID);
	m_Socket.Send(LogInMsg, LogInMsg.GetLength()*2);
...
void CSocketClientDlg::OnBnClickedButtonSend()
{
	...
	CString str; 
	UpdateData(TRUE);
	str = m_strMsgInput.GetString();
	int CODE = 1;
	CString Msg;
	Msg.Format(_T("%d %s"), CODE, str);
#ifdef _UNICODE
	m_Socket.Send(Msg, Msg.GetLength() * 2);
#elif
	m_Socket->Send(str, str.GetLength() * 3);
#endif
	m_strMsgInput.SetString(_T(""));
	UpdateData(FALSE);
	/*
	LogInMsg.Format(_T("%d %s"),CODE, UID);
	m_Socket.Send(LogInMsg, LogInMsg.GetLength()*2);
